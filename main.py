...
date = datetime.now()
date_fromated = date.strftime("%B %w, %Y")
app = Flask(__name__)
...
def admin_only(function):
    ...
    def wrapper(*args, **kwargs):
        if current_user.id != 1:
            abort(403, description="cant go there")
        else:
...
def show_post(post_id):
    ...
    if form.validate_on_submit() and form.validate_on_submit():
        ...
        flash("You've need to login if you want to comment bro")
        return redirect(url_for("login"))
